---
title: DigitalOcean
description: Review details on installation for Kubefirst Pro using DigitalOcean
keywords: [digitalocean, kubernetes, containers, clusters, kubefirst, konstruct] 
sidebar_position: 5
---

## Summary

Kubefirst and Kubefirst Pro support provisioning in DigitalOcean with three installation methods. Each method provides access to the same great experience in Kubefirst with access to Kubefirst Pro (the Kubefirst UI). Access to Kubefirst Pro includes your management cluster and the opportunity to upgrade to our Business tier for clusters at $0.33/hr.

## DigitalOcean Cloud Supported Installs

Kubefirst can be installed with DigitalOcean in three ways:

- Through the [DigitalOcean Marketplace](../digoc/marketplace/)
- Through the [Kubefirst CLI](../digoc/cli/) with MacOS, Linux, or Windows (using WSL and Ubuntu)
- With DigitalOcean and the [Kubefirst UI](../digoc/ui/).
  - One path uses a Kubefirst command _Recommended for users who do not have an existing cluster_
  - One path uses Helm and is _Recommended for users with an existing cluster for install_
...
The DigitalOcean install and provisioning process creates the following:

- a Kubernetes management cluster in the DigitalOcean cloud
