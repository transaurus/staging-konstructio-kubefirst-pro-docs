---
title: Civo CLI Prerequisites
description: Prerequisites for installation of Kubefirst with Civo and the CLI
...
This document walks through the prerequisites for installing **Kubefirst using Civo Cloud with the Kubefirst CLI**.
...
A Civo account with administrative access _(with permissions to create SSH Keys, S3 Buckets, Kubernetes Clusters, and a Network)_.

#### Civo API Key

Your [saved Civo API Key.](https://www.civo.com/docs/account/api-keys)
...
For information on Civo [check out their documentation on adding DNS.](https://www.civo.com/learn/configure-dns#adding-a-domain-name)
...
The installation of Kubefirst using Civo and the CLI has the following known limitations.
