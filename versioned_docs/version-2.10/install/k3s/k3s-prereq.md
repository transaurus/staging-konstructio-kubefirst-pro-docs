---
title: K3s Prerequisites
description: Prerequisites for installing Kubefirst with the CLI and K3s
keywords: [k3s, prerequisites, requirements, kubernetes, containers, clusters, kubefirst, konstruct]
sidebar_position: 1
---

## Summary

This document walks through the prerequisites for installing **Kubefirst using the CLI and K3s**.
...
For K3s we currently only offer support using Cloudflare for DNS.
...
- You need to have 3 virtual machines available to use for the K3s agents.
...
- Kubefirst is only available with Cloudflare for DNS.
- We do not currently offer support for installation with K3s using the Kubefirst UI.
