---
title: Google CLI Prerequisites
description: Prerequisites for installation of Kubefirst with Google Cloud and the CLI
keywords: [google cloud, prerequisites, requirements, kubernetes, containers, clusters, kubefirst, konstruct]
sidebar_position: 2
---

## Summary

This document walks through the prerequisites for installing **Kubefirst using Google Cloud with the Kubefirst CLI**.

## Install Assumptions

Before getting started make sure you are aware of the following:

- We assume that you already have a Google Cloud account and credentials.
...
Refer to the [Google documentation for configuring DNS in Google Cloud](https://cloud.google.com/dns/docs/overview/) for additional details.
...
- A Google Cloud account, where you are the account owner
- A Google Service Account created to use with Kubefirst
  - Your Google Cloud user must have permission to use the service account
  - A Key (JSON) associated with the service account.
- The Google Project-id
- The [Google Cloud CLI](https://cloud.google.com/cli?hl=en)

For more information refer to Google's [documentation on service accounts.](https://cloud.google.com/iam/docs/service-account-creds)

![Create your JSON Key](../../../img/google/google_createkey.png)
